...
describe("Chats Tests - Main tests for Chats"),
  async () => {
    it("Chat User A - Create Account", async () => {
      ...
      const didkey =
        await settingsProfileFirstUser.getCopiedDidFromStatusInput();
    ...
    after(async () => {
      // Save Cache Folder User B
      await grabCacheFolder("ChatUserB", USER_A_INSTANCE, "/.uplinkUserB");
