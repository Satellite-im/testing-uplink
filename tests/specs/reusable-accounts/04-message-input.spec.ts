...
export default async function messageInputTests() {
  ...
  it("Chat Input Text - Validate users can send messages using the code language markdown", async () => {
    ...
    await chatsInputFirstUser.switchToOtherUserWindow();
    await chatsInputFirstUser.typeCodeOnInputBar("JavaScript", "let a = b;");
    await chatsInputFirstUser.clickOnSendMessage();
    ...
    await expect(codeMessageTextSent).toEqual("let a = b;");
    ...
    await expect(codeMessageTextReceived).toEqual("let a = b;");
  });
  ...
  it("Chat Input Text - Code Markdown - User can copy the message from the code block", async () => {
    ...
    await expect(chatsInputFirstUser.inputText).toHaveText("let a = b;");
