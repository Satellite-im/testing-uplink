...
let createPinSecondUser = new CreatePinScreen();
let createUserFirstUser = new CreateUserScreen();
let createUserSecondUser = new CreateUserScreen();
let friendsScreenFirstUser = new FriendsScreen();
let friendsScreenSecondUser = new FriendsScreen();
let welcomeScreenFirstUser = new WelcomeScreen();
let welcomeScreenSecondUser = new WelcomeScreen();
...
export async function grabCacheFolder(
  username: string,
  cacheDir: string = "./uplink",
) {
  const source = homedir() + cacheDir;
...
export async function saveFileOnWindows(
  ...
  const editInput = await driver.$("/Window/Pane[1]/ComboBox[1]/Edit");
...
export async function selectFileOnWindows(
  ...
  // Wait for Save Panel to be displayed
  const listView = await driver.$("~listview");
  await listView.waitForExist({ timeout: 25000 });

  // Type file location and hit enter
  const editField = await driver.$("//Window/ComboBox/Edit");
