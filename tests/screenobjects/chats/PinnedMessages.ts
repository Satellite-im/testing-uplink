...
export default class PinnedMessages extends UplinkMainScreen {
  ...
  async validateFirstPinnedMessageAttachmentFileIcon(timeout: number = 15000) {
    const attachmentIcon = await this.getPinnedMessageAttachmentFileIcon(0, 0);
    await attachmentIcon.waitForExist({ timeout: timeout });
  }
  ...
  async validateFirstPinnedMessageImageProfileIsShown(timeout: number = 15000) {
